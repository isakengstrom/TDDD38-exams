/*
 * program8_2.cc
 * Templated policy classes with class type template argument to Number 
 *  -> forces the user to instantiate the policy class
 */
#include <iostream>
#include <iomanip>
#include <cmath>
...
template <typename value_type, typename floating_type,
          class Rounding_Policy = Round_Towards_Zero<floating_type, value_type>>
class Number
{
public:
...
private:
    ...
    {
        return Rounding_Policy::round(val);
    }
};
...
void test_rounding(double value)
{
    ...
    static Number<int, double, Round_Up<double,int>> n1;
    static Number<int, double, Round_Down<double, int>> n2;
