// Another solution for Pawn movement. Some of you felt that modifying "has_moved" inside valid_move was bad design
// so this is also a (somewhat better) solution.
#include <memory>
#include <cmath>
#include <utility>
#include <iostream>
...
{
public:
    Chess_Piece(Square start, Move_Behavior * m): pos{start}, current_behavior{m} {}
    void move(Square next)
    {
        if (current_behavior->valid_move(pos, next))
        {
            pos = next;
            if ( auto p = dynamic_cast<Pawn_Behavior *>(current_behavior.get()) )
            {
                p->has_moved();
                if ( p->should_change(pos) )
                {
...
private:
    Square pos;
    std::unique_ptr<Move_Behavior> current_behavior;
};
