...
bool is_synonyms(string const& word, string const& other)
{
    ...
    {
        // 2. Find the first position where word and other have
        //    different characters
        auto pos{mismatch(begin(word), end(word), begin(other))};        
        if (pos.first != end(word))
        {
            // 3. If such a position is found, search for the next
            //    position where they differ
            ++pos.first;
            ++pos.second;
            pos = mismatch(pos.first, end(word), pos.second);

            // 4. If no other difference is found, return true
            return pos.first == end(word);
        }
    }
...
int main()
{
    ...
    transform(begin(text), end(text), begin(text), get_synonym);
